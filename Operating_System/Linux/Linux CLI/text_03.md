...
> :mag_right:  Например, egrep это тоже самое, что и grep E (помните, командная строка Linux регистрозависимая и команды grep e и grep E интерпретируются по разному).

- То есть, к примеру, две команды ниже эквивалентны и выводят все строки, в которых есть две подряд идущих буквы "p".

```
egrep p\{2} avocado.txt

grep -E p\{2} avocado.txt
```
---
photo
---
...
```
> :mag_right: Zgrep также работает с tar архивами, но ограничивается лишь выводом информации о том, нашла ли она соответствие или нет. Это замечание мы сделали потому, что чаще всего gzip-ом архивируются tar файлы.
...
> :mag_right: Если вы просто хотите узнать количество строк в текстовом файле, вы можете использовать команду wc с опцией ‘l’. В основном, она подсчитывает количество строк в файле.
```
wc -l avocado.txt
```
2. Подсчитайте количество слов в файле

> :mag_right: Если вы просто хотите узнать количество слов в текстовом файле, вы можете использовать команду wc с опцией ‘w’. В ней будет отображаться количество слов с пробелами.
```
wc -w avocado.txt
```
---
